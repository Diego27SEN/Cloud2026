...
- Los identificadores de UGS (project ID, organization ID) los vacía un filtro `clean` de git antes
  de que entren, y un hook de `pre-commit` bloquea el commit si el filtro no está activo o si se
  cuela un identificador con valor. Ambos viven en el repo pero git no los aplica solo: cada clon
  los activa una vez con `sh Tools/setup-git.sh`.
- El archivo de entorno (`ProjectSettings/Packages/com.unity.services.core/Settings.json`) no se
  versiona: Unity lo regenera al vincular el proyecto y guarda `EnvironmentId` como `Guid`, así que
  versionarlo vacío rompe la inicialización del Editor. El nombre del entorno que usa el juego no
  sale de ahí, sino de `SetEnvironmentName` en el bootstrap.
